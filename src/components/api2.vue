...
<template>
  <div class="text-center  bg-[#481800] p-8 rounded-md  font-bold">
    ...
    <div class="flex flex-wrap justify-center">
      <div class="info-container info-container1 bg-purple-500 p-6 m'4 sm:w-1/2 rounded-md">
        <p v-if="info.ISC">ISC: Listas {{ info.ISC.listas }}, Faltantes {{ info.ISC.faltantes }}</p>
        <img src="../assets/ISC.png" alt=" " width="200" height="100" class="w-50 mx-auto mt-4 mb-4">

        <label v-if="info.ISC">Porcentaje completado:{{ Math.round(((info.ISC.listas - info.ISC.faltantes) / info.ISC.listas) * 100) }}%</label>
   <br>
   <div class="md:flex flex-row mb-5 mt-5">
    ...
    <p v-if="info.ISC" :style="{ color: (info.ISC.faltantes === 0) ? 'green' : 'darkred' }"> Porcentaje Faltante: {{ Math.round((info.ISC.faltantes / info.ISC.listas) * 100) }}%</p>

      </div>

      <div class="info-container info-container2  p-6 m'4 sm: bg-pink-300 sm:w-1/2 rounded-md">
        <p v-if="info.IEM">IME: Listas{{ info.IEM.listas }}, faltantes {{ info.IEM.faltantes }}</p>
        <img src="../assets/IEM.png" alt=" " class="w-70 h-20 mx-auto mt-4">


        <label v-if="info.IEM">Porcentaje completado:{{ Math.round(((info.IEM.listas - info.IEM.faltantes) / info.IEM.listas) * 100) }}%</label>
   <br>
   <div class="md:flex flex-row mb-5 mt-5">
        <label class="md:px-2">0%</label>
        <div class="bg-gray-300 w-full rounded-lg mt-1 ">
    ...
    <p v-if="info.IEM" :style="{ color: (info.IEM.faltantes === 0) ? 'green' : 'darkred' }"> Porcentaje Faltante: {{ Math.round((info.IEM.faltantes / info.IEM.listas) * 100) }}%</p>
   ...
   <br>
      <div class="info-container info-container3 bg-yellow-500 p-6 m'4 sm:w-1/2 rounded-md">
        <p v-if="info.IER">IER: Listas{{ info.IER.listas }}, faltantes {{ info.IER.faltantes }}</p>
        <img src="../assets/IER.png" alt=" " class="w-70 h-20 mx-auto mt-4">
   ...
   <div class="md:flex flex-row mb-5 mt-5">
    ...
    <p v-if="info.IER" :style="{ color: (info.IER.faltantes === 0) ? 'green' : 'darkred' }"> Porcentaje Faltante: {{ Math.round((info.IER.faltantes / info.IER.listas) * 100) }}%</p>
      ...
      <div class="info-container info-container4 bg-green-500 p-6 m'4 sm:w-1/2 rounded-md">
        <p v-if="info.IE">IE: Listas{{ info.IE.listas }}, faltantes {{ info.IE.faltantes }}</p>
        <img src="../assets/IE.png" alt=" " class="w-70 h-20 mx-auto mt-4">
   ...
   <div class="md:flex flex-row mb-5 mt-5">
    ...
    <p v-if="info.IE" :style="{ color: (info.IE.faltantes === 0) ? 'green' : 'darkred' }"> Porcentaje Faltante: {{ Math.round((info.IE.faltantes / info.IE.listas) * 100) }}%</p>
      ...
      <div class="info-container info-container5 bg-blue-500 p-6 m'4 sm:w-1/2 rounded-md">
        <p v-if="info.II">II: Listas{{ info.II.listas }}, faltantes {{ info.II.faltantes }}</p>
        <img src="../assets/II.png" alt=" " class="w-70 h-20 mx-auto mt-4">
